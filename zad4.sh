#!/usr/bin/env bash
#
# Systemy operacyjne 2 – laboratorium nr 2
#
# Celem zajęć jest nabranie doświadczenia w podstawowej pracy z powłoką Bash,
# w szczególności w nawigowaniu po drzewie katalogów i sprawdzaniu uprawnień.
# Proszę unikać wykorzystywania narzędzia `find` w ramach bieżących zajęć.
#
# Nie przywiązujemy wagi do środowiska roboczego – zakładamy, że jego pliki,
# inne niż te podane wprost w treści zadań, mogą ulec zmianie, a przygotowane
# rozwiązania nadal powinny działać poprawnie (robić to, o czym zadanie mówi).
#
# Wszystkie chwyty dozwolone, ale ostatecznie w wyniku ma powstać tylko to,
# o czym mowa w treści zadania – tworzone samodzielnie ewentualne tymczasowe
# pliki pomocnicze należy usunąć.
#

#
# Zadanie 4.
# Wyświetlić połączoną zawartość wszystkich plików z katalogu `dane/icao/`,
# których zawartości nie możemy zmienić (brak prawa do zapisu).
# Kolejność łączenia plików nie ma znaczenia.
#

for file in dane/icao/*; do
    if [ -f "${file}" ] && [ ! -w "${file}" ]; then # -f sprawdza czy plik jest zwykłym plikiem, a nie plikiem katalogowym
        cat "${file}"                               # ! -w sprawdza czy nie mamy prawa do zapisu w pliku (w - write), cat- wyświetla zawartość pliku
    fi
done