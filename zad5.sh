...
for file in dane/icao/*; do
    file_name=$(basename "$file")                 # pobranie nazwy pliku bez ścieżki (wyodrębnienie nazwy pliku z pełnej ścieżki)
    
    if [ ! -e "dane/backup/$file_name" ]; then    # sprawdzenie czy pobrany plik nie istnieje w katalogu dane/backup
        echo "$file_name"
    fi
