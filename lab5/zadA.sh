...
grep -E '^[0-9]+;' dodatkowe/listaSluchaczy_E07-19x.csv | cut -d';' -f2 | sed -E 's/^.*([0-9]{6})$/\1@student.pwr.edu.pl/'
# wybranie wierszy, które zaczynają się od cyfry, pomijając nagłówki
# wyciągnięcie drugiej kolumny przy użyciu cut
# włączenie rozszerzonego wyrażenia regularnego użycie operatora substytucji do użycia wyniku z pierwszej grupy i złączenia go z drugą grupą
# pobranie jak największej ilości znaków do końca linii, wyodbrębnienie 6 ostatnich ($), które są cyframi i przekazanie do grupy drugiej w miejsce 1