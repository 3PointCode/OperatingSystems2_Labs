...
for file in dane/pierwiastki/*; do
    if [ -L "$file" ]; then     # sprawdzenie czy plik jest dowiązaniem miękkim
        if [ -e "$file" ]; then      # sprawdzenie czy plik na który wskazuje dowiązanie istnieje
            realpath "$file"    # wyświetlenie kanonicznej ścieżki do pliku na które wskazuje dowiązanie
        fi
    fi
done