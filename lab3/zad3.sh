...
for file in dane/pierwiastki/*; do
    if [ -L "$file" ]; then     # sprawdzenie czy plik jest dowiązaniem symbolicznym
        if [ ! -e "$file" ]; then   # sprawdzenie czy plik wskazywany przez dowiązanie istnieje
            basename "$file"    # wyświetlenie nazwy wiszącego dowiązania
        fi
    fi
done